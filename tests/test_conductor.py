...
class TestRunConductor:
    ...
    def test_successful_decomposition(
        self, mock_run, store: TaskStore, config: Config, tmp_path: Path
    ):
    ...
    @patch("warchief.conductor.subprocess.run")
    def test_conductor_failure_returns_empty(
        self, mock_run, store: TaskStore, config: Config, tmp_path: Path
    ):
    ...
    @patch("warchief.conductor.subprocess.run")
    def test_conductor_invalid_json_returns_empty(
        self, mock_run, store: TaskStore, config: Config, tmp_path: Path
    ):
